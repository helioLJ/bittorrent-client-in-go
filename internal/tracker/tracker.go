...
func (t *Tracker) httpAnnounce(downloaded, uploaded, left int64) ([]net.Addr, error) {
	...
	logger.Debug("Raw peers data length: %d bytes", len(trackerResp.Peers))

	parsedPeers, err := parsePeers([]byte(trackerResp.Peers))
	if err != nil {
...
// MultiTracker represents a collection of trackers
...
// NewMultiTracker creates a new MultiTracker instance
...
// AddTracker adds a tracker to the MultiTracker instance
func (mt *MultiTracker) AddTracker(t *Tracker) {
	mt.trackers = append(mt.trackers, t)
}

// Announce sends an announce request to all trackers in the MultiTracker instance
